...
class WebApiTestCase(unittest.TestCase):
    ...
    def test_family_category_binding_preview_and_apply_confirm_semantic_mapping(self):
        create_family = self.client.post("/api/v1/families", json={"name": "Семья связи категорий"})
        self.assertEqual(create_family.status_code, 201)
        family_id = int(create_family.json()["id"])

        second_client = TestClient(app)
        second_email = f"category-binding-{uuid4().hex[:8]}@example.com"
        second_register = second_client.post(
            "/api/v1/auth/register",
            json={"email": second_email, "password": "AnotherPass123"},
        )
        ...
        preview_payload = {
            ...
            "category_names": ["Продукты", "Еда"],
        }
        preview = second_client.post(
            f"/api/v1/families/{family_id}/categories/bindings/preview",
            json=preview_payload,
        )
        self.assertEqual(preview.status_code, 200)
        preview_payload_response = preview.json()
        self.assertTrue(preview_payload_response["can_apply"])
        self.assertGreaterEqual(preview_payload_response["candidate_count"], 2)
        self.assertGreaterEqual(preview_payload_response["new_binding_count"], 2)

        member_cannot_apply = second_client.post(
            f"/api/v1/families/{family_id}/categories/bindings",
            json=preview_payload,
        )
        self.assertEqual(member_cannot_apply.status_code, 403)
        ...
        self.assertEqual(applied.status_code, 200)
        self.assertGreaterEqual(applied.json()["applied_bindings_count"], 2)

        audit_response = self.client.get(f"/api/v1/families/{family_id}/categories/audit")
        self.assertEqual(audit_response.status_code, 200)
        groceries_group = next(item for item in audit_response.json()["category_groups"] if item["semantic_key"] == "groceries")
        self.assertEqual(groceries_group["status"], "confirmed")
        self.assertGreaterEqual(groceries_group["confirmed_bindings_count"], 2)
    ...
    def test_family_dashboard_forecast_aggregates_family_budgets_for_all_members(self):
        ...
        family_dashboard_for_member = second_client.get(f"/api/v1/families/{family_id}/dashboard")
        self.assertEqual(family_dashboard_for_member.status_code, 200)
        member_payload = family_dashboard_for_member.json()
        self.assertEqual(member_payload["forecast"]["monthly_budget"], 500.0)
        self.assertEqual(member_payload["forecast"]["budget_remaining"], 400.0)
        self.assertEqual(member_payload["forecast"]["projected_balance"], 500.0)

        second_client.close()

    def test_family_dashboard_forecast_sums_budgets_by_shared_category_without_double_counting_expenses(self):
        create_family = self.client.post("/api/v1/families", json={"name": "Семья общий бюджет"})
        self.assertEqual(create_family.status_code, 201)
        family_id = int(create_family.json()["id"])

        second_client = TestClient(app)
        second_email = f"family-shared-budget-{uuid4().hex[:8]}@example.com"
        second_register = second_client.post(
            "/api/v1/auth/register",
            json={"email": second_email, "password": "AnotherPass123"},
        )
        ...
        owner_income = self.client.post(
            ...
            json={
                ...
                "category_name": income_category,
                "amount": 2000.0,
                "comment": "Owner income shared budget",
                "date": "2026-04-01",
            },
        )
        self.assertEqual(owner_income.status_code, 201)
        ...
        member_budget = second_client.post(
            "/api/v1/budgets",
            json={
                "category_id": expense_category["id"],
                "amount": 700.0,
        ...
        self.assertEqual(member_budget.status_code, 201)
        ...
        self.assertEqual(payload["forecast"]["monthly_budget"], 1200.0)
        self.assertEqual(payload["forecast"]["current_expenses"], 1000.0)
        self.assertEqual(payload["forecast"]["budget_remaining"], 200.0)
        self.assertEqual(payload["forecast"]["projected_balance"], 800.0)

        second_client.close()

    def test_family_daily_budget_forecast_skips_today_after_family_spend(self):
        today = datetime.now()
        days_in_month = calendar.monthrange(today.year, today.month)[1]
        remaining_days_after_today = days_in_month - today.day

        create_family = self.client.post("/api/v1/families", json={"name": "Семья daily forecast"})
        self.assertEqual(create_family.status_code, 201)
        family_id = int(create_family.json()["id"])

        second_client = TestClient(app)
        second_email = f"family-daily-forecast-{uuid4().hex[:8]}@example.com"
        second_register = second_client.post(
            "/api/v1/auth/register",
            json={"email": second_email, "password": "AnotherPass123"},
        )
        ...
        income = self.client.post(
            ...
            json={
                ...
                "category_name": income_category,
                "amount": 10000.0,
                "comment": "Owner income daily forecast",
        ...
        today_expense = second_client.post(
            "/api/v1/transactions",
            json={
                "type": "expense",
                "category_name": "Продукты",
                "amount": 1000.0,
                "comment": "Member daily forecast today",
                "date": today.strftime("%Y-%m-%d"),
            },
        )
        self.assertEqual(today_expense.status_code, 201)

        family_dashboard = self.client.get(f"/api/v1/families/{family_id}/dashboard")
        self.assertEqual(family_dashboard.status_code, 200)
        payload = family_dashboard.json()

        expected_budget_remaining = float(remaining_days_after_today * 100)
        self.assertEqual(payload["forecast"]["budget_remaining"], expected_budget_remaining)
        self.assertEqual(payload["forecast"]["projected_balance"], float(10000 - 1000 - expected_budget_remaining))

        second_client.close()

    def test_personal_accounts_and_dashboard_do_not_leak_between_family_members(self):
        create_family = self.client.post("/api/v1/families", json={"name": "Семья чужие счета"})
        self.assertEqual(create_family.status_code, 201)
        family_id = int(create_family.json()["id"])
    ...
    def test_family_capital_target_can_use_another_members_account_and_history_is_shared(self):
        ...
        create_family = self.client.post("/api/v1/families", json={"name": "Семья общий счёт"})
        self.assertEqual(create_family.status_code, 201)
        family_id = int(create_family.json()["id"])
