...
export function TransactionsPage() {
  ...
  function submitForm(event: FormEvent<HTMLFormElement>) {
    ...
    const templateName = recurringName.trim() || comment.trim() || filteredCategories.find((item) => String(item.id) === categoryId)?.name || "";
          ...
          <label className="filter-check">
            <input
              ...
              onChange={(event) => {
                ...
                if (checked && !recurringName.trim()) {
                  setRecurringName(comment.trim());
                }
              }}
              type="checkbox"
            />
