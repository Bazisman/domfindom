...
def mysql_strict_transactions_enabled(config=settings) -> bool:
    return bool(
        getattr(config, "mysql_strict_write_transactions_enabled", False)
        and mysql_shadow_write_enabled(config)
    )
...
def require_mysql_transaction_shadow_write_success(
    result: Dict[str, Any],
    operation: str,
    config=settings,
) -> None:
    if not mysql_strict_transactions_enabled(config):
        return
    mysql_result = (result.get("results") or {}).get("mysql") or {}
    if result.get("status") == "ok" and mysql_result.get("status") == "ok":
        return
