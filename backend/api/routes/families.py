...
def _normalize_email(value: str) -> str:
    ...
    if "@" not in normalized or "." not in normalized.split("@")[-1]:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_CONTENT, detail="РќРµРєРѕСЂСЂРµРєС‚РЅС‹Р№ email")
    return normalized
...
def _require_family_role(family_id: int, user_id: int):
    ...
    if not membership:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="РЎРµРјСЊСЏ РЅРµ РЅР°Р№РґРµРЅР° РёР»Рё РґРѕСЃС‚СѓРї Р·Р°РїСЂРµС‰РµРЅ")
    return membership
...
def _validate_member_management_rules(
    ...
    if target_role == "owner":
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="РќРµР»СЊР·СЏ РёР·РјРµРЅРёС‚СЊ РІР»Р°РґРµР»СЊС†Р° СЃРµРјСЊРё")
    if actor_role != "owner":
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="РќРµРґРѕСЃС‚Р°С‚РѕС‡РЅРѕ РїСЂР°РІ")
    if actor_user_id == target_user_id:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="РќРµР»СЊР·СЏ РёР·РјРµРЅРёС‚СЊ СЃРѕР±СЃС‚РІРµРЅРЅСѓСЋ СЂРѕР»СЊ СЌС‚РёРј РґРµР№СЃС‚РІРёРµРј")
    ...
    try:
        ...
        if str(exc) == "user_already_member":
            raise HTTPException(
                ...
                detail="РџРѕР»СЊР·РѕРІР°С‚РµР»СЊ СѓР¶Рµ СЃРѕСЃС‚РѕРёС‚ РІ СЃРµРјРµР№РЅРѕРј Р±СЋРґР¶РµС‚Рµ",
            ) from exc
        raise

    return FamilyInviteCreateResponse(
        message="РџСЂРёРіР»Р°С€РµРЅРёРµ СЃРѕР·РґР°РЅРѕ.",
        family_id=family_id,
        email=invited_email,
        role=payload.role,
...
def accept_family_invite(payload: FamilyInviteAcceptPayload, current_user=Depends(require_user)) -> FamilyInviteAcceptResponse:
    ...
    if not accepted:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="РРЅРІР°Р№С‚ РЅРµРґРµР№СЃС‚РІРёС‚РµР»РµРЅ РёР»Рё РёСЃС‚РµРє")

    return FamilyInviteAcceptResponse(
        message="Р’С‹ РїРѕРґРєР»СЋС‡РµРЅС‹ Рє СЃРµРјРµР№РЅРѕРјСѓ Р±СЋРґР¶РµС‚Сѓ.",
...
def accept_family_invite_by_id(invite_id: int, current_user=Depends(require_user)) -> FamilyInviteAcceptResponse:
    if invite_id <= 0:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_CONTENT, detail="РќРµРєРѕСЂСЂРµРєС‚РЅС‹Р№ РёРґРµРЅС‚РёС„РёРєР°С‚РѕСЂ РїСЂРёРіР»Р°С€РµРЅРёСЏ")
    accepted = auth_service.accept_family_invite_by_id(invite_id=invite_id, user_id=int(current_user["id"]))
    if not accepted:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="РџСЂРёРіР»Р°С€РµРЅРёРµ РЅРµРґРµР№СЃС‚РІРёС‚РµР»СЊРЅРѕ РёР»Рё РёСЃС‚РµРєР»Рѕ")
    return FamilyInviteAcceptResponse(
        message="Р’С‹ РїРѕРґРєР»СЋС‡РµРЅС‹ Рє СЃРµРјРµР№РЅРѕРјСѓ Р±СЋРґР¶РµС‚Сѓ.",
        family_id=int(accepted["family_id"]),
        family_name=str(accepted["family_name"]),
...
def decline_family_invite_by_id(invite_id: int, current_user=Depends(require_user)) -> FamilyActionResponse:
    ...
    if not declined:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="РџСЂРёРіР»Р°С€РµРЅРёРµ РЅРµРґРµР№СЃС‚РІРёС‚РµР»СЊРЅРѕ РёР»Рё СѓР¶Рµ РѕР±СЂР°Р±РѕС‚Р°РЅРѕ")
    return FamilyActionResponse(message="РџСЂРёРіР»Р°С€РµРЅРёРµ РѕС‚РєР»РѕРЅРµРЅРѕ.")
...
def update_family_member_role(
    ...
    if family_id <= 0 or member_user_id <= 0:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_CONTENT, detail="РќРµРєРѕСЂСЂРµРєС‚РЅС‹Рµ РїР°СЂР°РјРµС‚СЂС‹")
    ...
    try:
        ...
        if str(exc) == "owner_role_locked":
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="РќРµР»СЊР·СЏ РёР·РјРµРЅРёС‚СЊ СЂРѕР»СЊ РІР»Р°РґРµР»СЊС†Р°") from exc
        raise

    if not updated:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="РЈС‡Р°СЃС‚РЅРёРє РЅРµ РЅР°Р№РґРµРЅ")
    return FamilyActionResponse(message="Р РѕР»СЊ СѓС‡Р°СЃС‚РЅРёРєР° РѕР±РЅРѕРІР»РµРЅР°.")
...
def remove_family_member(
    ...
    try:
        ...
        if str(exc) == "owner_cannot_be_removed":
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="РќРµР»СЊР·СЏ СѓРґР°Р»РёС‚СЊ РІР»Р°РґРµР»СЊС†Р° СЃРµРјСЊРё") from exc
        raise

    if not removed:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="РЈС‡Р°СЃС‚РЅРёРє РЅРµ РЅР°Р№РґРµРЅ")
    return FamilyActionResponse(message="РЈС‡Р°СЃС‚РЅРёРє РёСЃРєР»СЋС‡РµРЅ РёР· СЃРµРјСЊРё.")
